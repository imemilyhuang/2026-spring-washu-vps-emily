(function () {
  ...
  function showChapterOverview(chapter, targetIndex) {
    ...
    if (els.chapterOverviewCitiesWrap) {
      ...
      if (!isEnd) {
        ...
        block.className = "chapter-overview-cities-block";

        const label = document.createElement("p");
        label.className = "chapter-overview-label";
        label.textContent = "Chapter Overview";

        const tagsWrap = document.createElement("div");
        tagsWrap.className = "chapter-overview-tags";
        ...
        (ch.cities || []).forEach((ref) => {
          ...
          btn.type = "button";
          btn.className = "chapter-overview-city-tag";
        ...
        els.chapterOverviewCitiesWrap.appendChild(block);
      }
    }
  ...
  function showMotifView(motifName) {
    ...
    items.forEach(({ city, motif: m }) => {
      ...
      const iconHtml = iconSrc
        ...
        : "";
      const description = (m && m.description) ? escapeHtml(m.description) : "";
      const quotes = (m && m.quotes && m.quotes.length) ? m.quotes.map((q) => `"${escapeHtml(q)}"`).join(" ") : "";
      const card = document.createElement("button");
      card.type = "button";
      card.className = "motif-card";
      card.innerHTML = `
        <div class="motif-card-header">
          <div class="motif-card-col-icon">${iconHtml}</div>
          <div class="motif-card-col-text">
            <div class="motif-card-title">${escapeHtml(city.name)}</div>
            <p class="motif-card-subtitle">${escapeHtml(subtitle)}</p>
          </div>
        </div>
        ${description ? `<p class="motif-card-description">${description}</p>` : ""}
        ${quotes ? `<p class="motif-card-quotes">${quotes}</p>` : ""}
      `;
  ...
  function renderNavOverlay() {
    ...
    chapters.forEach((chapter) => {
      ...
      section.className = "nav-chapter-section";
      section.dataset.chapterId = chapter.id;

      const label = document.createElement("p");
      label.className = "nav-chapter-label";
      label.textContent = chapter.title || "Chapter Overview";

      const tagsWrap = document.createElement("div");
      tagsWrap.className = "nav-chapter-tags";

      const readLink = document.createElement("button");
      readLink.type = "button";
      readLink.className = "nav-chapter-city-tag";
      readLink.textContent = "Chapter Overview";
      ...
      (chapter.cities || []).forEach((ref) => {
        ...
        const btn = document.createElement("button");
        btn.type = "button";
        btn.className = "nav-chapter-city-tag";
        btn.textContent = ref.name;
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          currentIndex = idx;
      ...
      els.navOverlayList.appendChild(section);
    });
  }
